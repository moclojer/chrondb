...
(defn native-image
  "Build a native image for ChronDB.
   ...
   - :simplified - Boolean flag to use minimal configuration (default: false)"
  [{:keys [verbose static output extra_flags enable_url_protocols simplified]
    :or {verbose false
         static false
  ...
  (let [graal-configs (if simplified
                        ;; Modo simplificado com configuração mínima - para CI e ambientes restritos
                        ["--no-fallback"
                         "--verbose"
                         ...
                         "-H:+AddAllCharsets"
                         ;; Desabilitar requisitos para ScopedMemoryAccess
                         "--report-unsupported-elements-at-runtime"
                         "--initialize-at-run-time=jdk.internal.misc.ScopedMemoryAccess"]
                        ...
                        ["--no-fallback"
                         ...
                         "--initialize-at-run-time=org.apache.lucene.index.ConcurrentMergeScheduler"
                         "--initialize-at-run-time=org.apache.lucene.index.SegmentReader"
                         "--initialize-at-build-time=org.apache.lucene.document.Field$Store"
                         "--initialize-at-build-time=org.apache.lucene.document.Field"
                         "--initialize-at-build-time=org.apache.lucene.document.Document"
                         "--initialize-at-build-time=org.apache.lucene.document"
                         ...
                         "--link-at-build-time=jdk.internal.misc.ScopedMemoryAccess"
                         "-H:JNIConfigurationFiles=graalvm-config/jni-config.json"
                         "--initialize-at-build-time=org.apache.lucene.search.BooleanClause$Occur"])
