...
(defn- process-group-columns
  "Processa as colunas de um grupo"
  [group columns]
  (reduce
    (fn [acc col-def]
      (case (:type col-def)
...
(defn handle-select-case
  "Manipula o caso SELECT de uma consulta SQL"
  [storage out parsed]
...
(defn handle-insert-case
  "Manipula o caso INSERT de uma consulta SQL"
...
(defn handle-update-case
  "Manipula o caso UPDATE de uma consulta SQL"
  [storage index out parsed]
  (try
    ...
    (let [where-conditions (:where parsed)
      ...
      (if (and (seq where-conditions) (seq updates))
        (let [_ (log/log-info (str "Pesquisando documentos correspondentes a: " where-conditions))
...
(defn handle-delete-case
  "Manipula o caso DELETE de uma consulta SQL"
  [storage index out parsed]
